const { Product, Designer, UserActivity, Sale, SaleItem, PurchaseOrder, Customer, RepairOrder, Vendor } = require('../models');
...
exports.getSystemAlerts = async (req, res) => {
    try {
        ...
        alerts.vendorPayments = pendingPayments.map(po => {
            ...
            return {
                ...
                message: `Payment to ${po.vendor?.name || 'Vendor'} - ₹${amountDue.toFixed(2)} due`,
        ...
        // Calculate summary
        const allAlerts = [...alerts.lowStock, ...alerts.overdueRepairs, ...alerts.vendorPayments];
...
// @desc    Get vendor pending payment alerts
// @route   GET /api/dashboard/alerts/vendor-payments
// @access  Public
exports.getVendorPendingPayments = async (req, res) => {
    try {
        const today = new Date();
        const pendingPayments = await PurchaseOrder.findAll({
            where: {
        ...
        console.error('Error fetching vendor payment alerts:', error);
        res.status(500).json({ 
            message: 'Failed to fetch vendor payment alerts',
            error: error.message 
        });
    }
};
